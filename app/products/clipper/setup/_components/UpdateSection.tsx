...
import { BLOCK, BODY_CLASS, CARD_BORDER, CHIP_LIGHT, H2_CLASS } from "./skin";
...
export function UpdateSection({ terminalInstruction }: { terminalInstruction: ReactNode }) {
  return (
    <section id="update" className={`${BLOCK} bg-white px-4 py-14 sm:px-10 sm:py-20 scroll-mt-24`}>
      <div className="mx-auto max-w-3xl flex flex-col gap-10">
        <AnimatedSection className="max-w-2xl flex flex-col gap-3">
          <span className={`${CHIP_LIGHT} self-start`}>Sudah punya Clipper?</span>
          <h2 className={H2_CLASS}>Update ke versi terbaru.</h2>
          <p className={BODY_CLASS}>
            Lisensi, pengaturan, dan klip kamu <b className="text-[#191919]">aman semua</b> — update cuma
        ...
        <AnimatedSection className={`rounded-[24px] ${CARD_BORDER} bg-[#EFEFEC] p-7 flex flex-col gap-3`}>
          ...
          <h3 className="text-base font-semibold text-[#191919]">Jalankan urutan ini</h3>
          <p className="text-[15px] text-[#5F5F5C] leading-relaxed">{terminalInstruction}</p>
